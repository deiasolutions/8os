...
supersedes: 8OS-BLOCK-1-SPEC v1.1.0
superseded_by: null
depends_on: 8OS-KERNEL-SPEC v0.2.0; AXIOM-8-AMENDMENT-PROPOSAL-v0_1; AXIOM-8-AUDIT-v0_1; BLOCK-5.0-PHASE-A-PRIME-REPORT
revisit_when: implementation surfaces a contradiction with the nine axioms or with this representation, or a multi-factory deployment exposes a coordination need this representation cannot express
provenance: Block 4 architecture conversation between Q88N and Claude derived the v1.1 commitments captured in the decisions log dated 2026-04-28. v1.1 consolidated v1.0.0 + v1.0.1-partial + Block 2.7 corrections + Block 2.8 amendments. v1.2 (Block 5.0, the v1.2 amendment cycle) ratifies axiom 8 (Reflexivity) into the kernel spec as v0.2 and lands the consequent corrections in this representation: §3.17 `kernel.reindex` mode rename (`mode: "full"` → `mode: "rebuild"`), §3.17 tightening to mandate tier-3 event emission in rebuild mode (closes audit target 4 via two-phase commit pattern), and cross-references to axiom 8 in §2.4 (kernel sovereignty), §3.5 (calibrator capability-update), and §3.17 (reindex). Implementation guidance derives from BLOCK-5.0-PHASE-A-PRIME-REPORT.
---

# 8OS Block 1 Specification v1.2
...
What is implemented and shipping at v1.0.1-partial: the kernel ABI per the eight axioms, the operations from the v1.0.1-partial SDK contract, the factory + Anthropic Python bridge, the SCAN dogfood with round-trip recomposition, calibration policies and predictors, and the single-writer JSONL ledger.
...
v1.1 makes the split structural. §3 specifies the SDK operations — all inside ops. §11 specifies `kernel.outside.http` and the outside-call governance machinery. The two sections are parallel in form (op contract, atomicity, error codes, event shape) but separate by intent. A reader asking "how many things does the kernel expose" finds two distinct sets — the SDK operations as inside ops and a small set of outside-call primitives — with the separation reflecting the underlying architecture, not just an editorial choice.
...
- **Inside ops** — the SDK operations specified in §3. They manipulate state the kernel owns.
...
Both categories share the same SDK shape (op contract, atomicity, error codes, tier 3 event emission). They differ in what they touch. The SDK operations never reach outside the kernel's ownership; the outside-call primitives always do.

The inside/outside split is structural per axiom 0. v1.1 makes it visible in the spec organization. A reader asking "what does the kernel expose" finds the inside SDK and a small set of outside-call primitives, with the separation reflecting the underlying architecture.
...
## Section 3 — The SDK operations

This section specifies the inside operations that constitute the v1.1 SDK contract. The contract is self-contained here; a reader does not need to chain through earlier specs to assemble it.

The SDK operations are v1.0.1-partial's operations preserved verbatim plus one new operation `kernel.ir.cancel`. Outside-call primitives (`kernel.outside.http` and related machinery) are specified separately in §11; they are not part of the SDK operation set, by structural intent per §0.2.

### 3.0 The SDK operations at a glance
...
**Purpose**: Regenerate indexes from (I, R) records on disk. With `mode: "check"`, validate that on-disk records are schema-conformant and that indexes are consistent.
...
**Mode-name reconciliation (v1.2):** v1.2 canonicalizes the mode value as `"rebuild"`. Prior to v1.2, the binary accepted `mode: "full"` while this spec documented `mode: "rebuild"` and the CLI flag was `--rebuild` — three names for the same mode (the "mode-name drift" surfaced in `BLOCK-5.0-PHASE-A-PRIME-REPORT` Pre-finding 1). v1.2 hard-renames the binary's accepted value to `"rebuild"`. There is no deprecation alias; callers using `mode: "full"` against a v1.2+ binary receive `SCHEMA_INVALID`. The CLI flag remains `--rebuild`.

**Output**:
```json
{ "mode": "rebuild" | "check",
  "input_set_hash": "<hash-string>",
  "indexes_written": <int>,        // rebuild only
  "drift_detected": false,         // check only
  "tier3_event_id": "<id>"|null }
```

**Atomicity**: rebuild mode is two-phase per the axiom-8 carve-out below. Check mode is read-only and emits no events.

**Validation**: per v1.0.1-partial, `kernel.reindex` in check mode enforces presence of `authored_via` on every record. v1.1 extended validation to include the new base fields specified in §4 (`data_classification` validity if present, `domain` inheritance correctness, `visible_when` predicate parseability) and the status enum values from §5. v1.2 preserves these checks unchanged.

**Indexes**: regenerates the kernel's regenerable index set. The v1.1 / v1.2 roster preserves v1.0.1-partial's indexes (`id-to-path`, `path-to-id`, `scope-to-ids`, `tier-to-ids`, `projection-to-ids`, `temporal`, `deps-forward`, `deps-reverse`, `_checksum`, `resolver-to-events`, `calibration-corpus`, and the bridges-and-resolvers index per v0.2) and adds the v1.1 indexes `policy-evaluations` (for cached policy results), `lease-holders` (for current lease state), and `payload-hash-to-events` (for outside-call deduplication). All indexes are regenerable from (I, R) records on disk.

**Errors**: `SCHEMA_INVALID`, `INDEX_DRIFT_DETECTED`, `RECORD_UNREADABLE`.

**Axioms**: 1, 3, 4, 6, **8**.

**Axiom 8 — rebuild-mode tier-3 event emission (v1.2 tightening, two-phase commit):**

Index regeneration is a kernel claim about the kernel's own state — the assertion *"the indexes are now consistent with the underlying records."* Per axiom 8 (Reflexivity), this claim must be (I, R)-formed. v1.2 tightens `kernel.reindex` rebuild mode to mandate emission of a tier-3 event recording the regeneration; the event is the (I, R) form of the kernel's consistency claim.

The tier-3 event creates a recursion concern: tier-3 events are themselves records on disk (in JSONL ledger files), and the events index regenerated by reindex depends on the ledger. Naively appending the rebuild event into the ledger before regenerating the events index produces an immediate inconsistency on the next check-mode run. v1.2 specifies the two-phase commit pattern:

1. **Phase 1 (regenerate).** Regenerate all kernel indexes from records on disk. Compute `input_set_hash` over the input record set, *excluding* the tier-3 event the rebuild itself will emit.
2. **Phase 2 (record-the-claim).** After phase 1's atomic commit completes, append a tier-3 event to the ledger recording the rebuild — `event_type: "operation"`, `resolver_id: "kernel"`, `authored_via: kernel.self`, `authority_level: hard`, with `input_set_hash` and `indexes_updated` in the structured payload. Update the events index incrementally for this single new entry.

A subsequent `mode: "check"` finds the indexes consistent because phase 2's append is reflected by the incremental events-index update. The reindex event is itself an (I, R) on the graph, satisfying axiom 8.

**Carve-out classification.** Rebuild-mode emission is not a third axiom-8 carve-out (alongside bootstrap and policy-evaluation cache, per kernel spec v0.2). It is the standard tier-3 event-emission discipline applied to a kernel-internal op, with two-phase commit specified to handle the structural recursion that would otherwise bite. The rebuild claim is recordable on the same terms as any other resolution event; the only structural detail is that the event must be appended after the index regen, not before, to preserve consistency.

**Check-mode behavior unchanged.** `mode: "check"` remains read-only and emits no event. There is no kernel claim made by check mode; either the indexes match the records (no claim) or they do not (`INDEX_DRIFT_DETECTED` raises). The audit's discipline distinguishes *asserting consistency* (rebuild — a claim, must be recordable) from *verifying consistency* (check — a verification, not a claim).
...
**Axiom 8 (v1.2).** The cogito discipline is the structural form of axiom 8 (Reflexivity, kernel spec v0.2): every kernel claim about its own state is itself an (I, R) authored through `kernel.self`. Two structural carve-outs are named in kernel spec v0.2 §"Axiom 8 — Reflexivity" — bootstrap (`kernel.init`) and policy-evaluation cache (`op_pipeline._author_policy_evaluation`) — both of which preserve (I, R) shape, hard authority, `authored_via: kernel.self`, atomic commit, and tier-3 event emission while bypassing `kernel.ir.new`'s validation pipeline for principled reasons (validation cannot precede the records that define the validation rules; recursion-avoidance for policy-evaluation phase). Future kernel-internal ops that surface a similar bypass pattern MUST be documented as named carve-outs in the kernel spec, not silently bypassed. `kernel.reindex` rebuild mode is *not* a carve-out — its tier-3 event is emitted via the standard discipline using a two-phase commit (§3.17).
...
**Axiom 8 (v1.2).** Capability-update records are kernel claims about resolver capability vectors and are therefore subject to axiom 8 (Reflexivity). The calibrator is a userspace resolver: the kernel binary contains no calibrator implementation, only the vendored `kernel.calibrator` `_kernel.resolver` record describing it (per `init_op.py`). Userspace resolvers have no privileged write path available — their only authoring path is `kernel.ir.new` with `tier: 2, projection_types: [_kernel.capability-update]`, which routes through the standard validation pipeline (schema validation, hard-authority enforcement on `_kernel` scope, policy-evaluation phase, atomic commit, tier-3 op-event emission). Capability-update is the cleanest example of axiom-8 compliance by construction: the userspace boundary forces the calibrator through the standard SDK; no privileged bypass is even available. Code-side verification at Block 5.0 Phase A′ confirmed this discipline holds (see BLOCK-5.0-PHASE-A-PRIME-REPORT §"OPEN-Q-A1").
...
### 11.9 Why this is not another SDK op

Per §0.2, outside-call primitives are a separate category from the SDK operations. Counting `kernel.outside.http` as another SDK op would flatten the inside/outside distinction the kernel was built around. The split is structural, not editorial.

A reader asking "what does the kernel expose" finds the inside SDK ops and a small set of outside-call primitives (currently one). The category separation reflects the architecture: the kernel's surface to its own state and the kernel's surface to outside reality are different surfaces. Both are SDK-shaped; both are auditable; they are not the same thing.

This framing also generalizes cleanly. As `kernel.outside.file`, `kernel.outside.human`, etc. land in future versions, they join the outside-call category without contaminating the inside-op set. The inside ops are the contract for kernel state; the outside-call primitives are the contract for boundary crossing. Two contracts, parallel in shape, distinct in intent.
...
- **The kernel's regenerable indexes.** Every kernel index — those preserved from v1.0.1-partial and the v1.1 additions (`policy-evaluations`, `lease-holders`) — lives as a DuckDB table.
...
The SDK (§3) and the outside-call primitives (§11) are the contract. Whether the backend is JSONL files, DuckDB, SQLite, or something else is opaque to programs using the SDK.
...
2. Rebuild all v1.0.1-partial indexes against the DuckDB tables.
...
The SDK plus the outside-call primitives expose what observability tooling needs:
...
5. **DuckDB index materialization.** Build DuckDB tables from existing JSONL events; rebuild all v1.0.1-partial indexes; build the new v1.1 indexes; verify checksums.
...
v1.1 does not respec the operations preserved from v1.0.1-partial; their op contracts are inherited. v1.1 does add `kernel.ir.cancel` as a new operation, and extends the cost shape on existing ops to three-vector decomposition.
...
- The SDK operations (§3) including the new `kernel.ir.cancel`
...
*End of Block 1 specification v1.2. Authored 2026-04-28 by Q88N + Claude (v1.1 base); axiom-8 amendments authored 2026-05-02 by Q88N + Claude (Block 5.0). Ratified 2026-05-03 (Block 5.0 closure). Supersedes v1.1, v1.0.1-partial, and the predecessor chain consolidated into v1.1. Locks the v1.1 architectural commitments derived in the Block 4 architecture conversation, with the v1.2 axiom-8 tightening of §3.17 `kernel.reindex` per BLOCK-5.0-PHASE-A-PRIME-REPORT.*
